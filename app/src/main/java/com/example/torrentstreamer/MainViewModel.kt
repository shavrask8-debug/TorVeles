...
class MainViewModel(application: Application) : AndroidViewModel(application) {
    ...
    // Ініціалізація TMDB-обробників
    private val tmdbApi = TmdbApi.create()
    private val metadataFetcher = MetadataFetcher(application, tmdbApi, dao)
    private val activeFetches = ConcurrentHashMap<String, Boolean>()
    ...
    /**
     * Перевіряє наявність метаданих та запускає фонове завантаження / 6-місячне оновлення кешу
     */
    private fun triggerMetadataFetch(hash: String, title: String) {
        if (activeFetches.putIfAbsent(hash, true) == null) {
            viewModelScope.launch {
                try {
                    val existing = dao.getMetadata(hash).firstOrNull()
                    val sixMonthsMs = 180L * 24 * 60 * 60 * 1000

                    if (existing == null || (System.currentTimeMillis() - existing.fetchedAt) > sixMonthsMs) {
                        Log.i("MainViewModel", "Triggering TMDB metadata fetch for $title ($hash)")
                        metadataFetcher.fetchAndCache(hash, title)
                        refreshTorrents(showSpinner = false)
                    }
                } catch (e: Exception) {
                    Log.e("MainViewModel", "Error managing metadata fetching for $hash: ${e.message}")
                } finally {
                    activeFetches.remove(hash)
                }
            }
        }
    }
                    ...
                    if (activeHash != null) {
                        ...
                        val mappedList = list.map { torrent ->
                            ...
                            if (localPoster.isNullOrBlank()) {
                                triggerMetadataFetch(torrent.hash, torrent.title)
                            }
                            torrent.copy(
